...
?>
<div class="statistics-at-a-glance-wrapper">
    <!-- Key Statistics Metric Cards -->
    <?php if ($cardsBlock): ?>
        <div class="stats-overview-grid" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 40px;">
            <?php foreach (($cardsBlock['items'] ?? []) as $i => $item): 
                ...
                $icon = 'fa-users';
                if (stripos($label, 'Academic') !== false) {
                    $icon = 'fa-graduation-cap';
                } elseif (stripos($label, 'Professor') !== false) {
                    $icon = 'fa-university';
                } elseif (stripos($label, 'Student') !== false) {
                    $icon = 'fa-book';
                }
            ?>
    ...
    <div class="int-tab-bar" style="margin-bottom: 30px; display: flex; flex-wrap: wrap; gap: 8px;">
        <?php foreach ($charts as $i => $block): 
            ...
            if (!$chart) continue;
        ?>
            <button class="int-tab-btn <?= $i === 0 ? 'active' : '' ?>" onclick="switchAtAGlanceTab('chart-<?= $i ?>', this)" style="padding: 10px 18px; font-size: 13.5px; font-weight: 700;">
                <i class="fa fa-pie-chart" style="margin-right: 6px;"></i> <?= e($chart['title'] ?? '') ?>
            </button>
        <?php endforeach; ?>
    </div>
...
<script>
function switchAtAGlanceTab(panelId, btn) {
    document.querySelectorAll('.statistics-at-a-glance-wrapper .int-tab-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    
    document.querySelectorAll('.statistics-at-a-glance-wrapper .int-tab-panel').forEach(p => p.classList.remove('active'));
    ...
    const container = document.querySelector('.statistics-at-a-glance-wrapper');
